...
public class UNICODE_MessageBoxMenuPanelIII extends UNICODE_MenuPanel
{
    ...
    public void mouseClickedExtendedDefinition ( MouseEvent mEvent )
    {
        if ( getMenu ( ).getButtonList ( ).get ( 0 ).contains ( mEvent.getX ( ), mEvent.getY ( ) ) )
        {
			...
			UNICODE_MessageBoxPanel messageBoxPanel = ( UNICODE_MessageBoxPanel ) getCustomComponentList ( ).get ( 0 );
			JFrame frame = ( JFrame ) getCustomComponentList ( ).get ( 1 );
