...
public class UNICODE_MessageBoxMenuPanelXI extends UNICODE_MenuPanel
{
    ...
    public void mouseClickedExtendedDefinition ( MouseEvent mEvent )
    {
        ...
        if ( getMenu ( ).getButtonList ( ).get ( 3 ).contains ( mEvent.getX ( ), mEvent.getY ( ) ) )
        {
			...
			Thread thread = ( Thread ) getCustomComponentList ( ).get ( 5 );
			thread.start ( );
